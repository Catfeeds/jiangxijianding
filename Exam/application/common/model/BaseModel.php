...
//            }
     ...
     */
    public function BaseSelect($param = [], $field = [], $order = '', $limit = "", $group = '')
    {
        try {
            if ($message = checkArray($field, $param)) {
                throw new \exception($message);
            } else {
                $res['data'] = self::where($param)->field($field)->group($group)->order($order)->limit($limit)->select();
            }
        } catch (\exception $e) {
            $res['error'] = true;
            $res['message'] = $e->getMessage();
     ...
     * @return string
     */
    public function BaseSelectPage($paginate, $param = [], $field = [], $order = '', $group = '')
    {
        try {
            if ($message = checkArray($field, $param, $paginate)) {
                ...
                if (!empty($paginate[2])) {
                    ...
                    $count = self::where($param)->field($field)->group('')->count();
                    $res['data'] = self::where($param)->field($field)->group($group)->order($order)->paginate($listRows, $count, $config);
                } else {
                    list($listRows, $simple) = $paginate;
                    $count = self::where($param)->field($field)->group($group)->count();
                    $res['data'] = self::where($param)->field($field)->group($group)->order($order)->paginate($listRows, $count);
                }
            }
        } catch (\exception $e) {
            $res['error'] = true;
    ...
    public function BaseWithSelect($with, $param = [], $field = [], $order = '', $limit = "")
    {
        try {
            if (!empty($with) && !is_string($with)) {
                ...
                $res['data'] = self::with($with)->where($param)->field($field)->order($order)->limit($limit)->select();
            }
        } catch (\exception $e) {
            $res['error'] = true;
            $res['message'] = $e->getMessage();
     ...
     * @return string
     */
    public function BaseJoinSelect($alias, $join, $param = [], $field = [], $order = '', $limit = "", $group = '')
    {
        /**
         *  $join = array(
        ...
        try {
            if (empty($alias) || empty($join)) {
                ...
                $res['data'] = self::where($param)->alias($alias)->join($join)->field($field)->group($group)->order($order)->limit($limit)->select();
            }
        } catch (\exception $e) {
            $res['error'] = true;
            $res['message'] = $e->getMessage();
     ...
     * @return string
     */
    public function BaseJoinSelectPage($paginate, $alias, $join, $param = [], $field = [], $order = '', $group = '')
    {
        try {
            if (empty($alias) || empty($join)) {
                throw new \exception('请对当前表起别名,同时传递链接参数');
                ...
                if (!empty($paginate[2])) {
                    list($listRows, $simple, $config) = $paginate;
                    $res['data'] = self::where($param)->alias($alias)->join($join)->field($field)->order($order)->group($group)->paginate($listRows, $simple, $config);
                } else {
                    list($listRows, $simple) = $paginate;
                    $res['data'] = self::where($param)->alias($alias)->join($join)->field($field)->order($order)->group($group)->paginate($listRows, $simple);
                }
            }
        } catch (\exception $e) {
            $res['error'] = true;
    ...
    public function BaseSave($data, $allowfield = true, $key = 'id')
    {
        try {
            if (!is_array($data) || empty($data)) {
                ...
                $res = $this->allowField($allowfield)->isUpdate(false)->data($data, true)->save();
            }
        } catch (\exception $e) {
            $res['message'] = $e->getMessage();
            $res['error'] = true;
     ...
     * @return int|string
     */
    public function BaseSaveAll($arrdata, $update = true, $id = '')
    {
        try {
